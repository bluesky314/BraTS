...
#input skip, residual in mid, subpixel
...
gpu_number = "1"            # which gpu to run on( as str 0-2)
...
save_initial='unetbiasfull_trainfull'
import os
import torch
...
#             flairn=nib.load(flairn)
            ...
            flair = np.swapaxes(np.swapaxes(flair,0,1),1,2)[20:212,20:212,15:143]   # read image
            t1 = np.swapaxes(np.swapaxes(t1,0,1),1,2)[20:212,20:212,15:143]
            t1ce = np.swapaxes(np.swapaxes(t1ce,0,1),1,2)[20:212,20:212,15:143]
            t2 = np.swapaxes(np.swapaxes(t2,0,1),1,2)[20:212,20:212,15:143]
...
class BraTS_FLAIR_val(torch.utils.data.Dataset):  # Validation dataset getter
    ...
    def __getitem__(self, idx): 
        ...
        sample = {'img': torch.from_numpy(img), 'mask': seg.type(torch.ByteTensor),'seg_orig':seg_orig.type(torch.ByteTensor)}
...
class UNet(torch.nn.Module):        # input -> shrunk in 1/2 then transposed at end. Loss can be with orig seg label / shurken if remove transpose
    def __init__(self):
        ...
        self.down_block4 = UNet_down_block(524, 1028, True)
        self.max_pool = torch.nn.MaxPool3d(2, 2)

        self.mid_conv1 = torch.nn.Conv3d(1028, 1028, 3, padding=1)
        self.bn1 = torch.nn.BatchNorm3d(1028)
        self.mid_conv2 = torch.nn.Conv3d(1028, 1028, 3, padding=1)
        self.bn2 = torch.nn.BatchNorm3d(1028)
        self.mid_conv3 = torch.nn.Conv3d(1028, 1028, 3, padding=1)
        self.bn3 = torch.nn.BatchNorm3d(1028)

        self.up_block1 = UNet_up_block(1028, 1028, 524)
